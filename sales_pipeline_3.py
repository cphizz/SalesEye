...
# Set to False to disable transcript saving entirely
SAVE_TRANSCRIPT = False

# PII patterns to scrub before any processing or saving
PII_PATTERNS = [
    # Names (basic titles)
    r'\b(mr|mrs|ms|dr|miss)\s+[A-Z][a-z]+\b',
    # Phone numbers
    r'\b(\+?1?[\s.-]?)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}\b',
    # Email addresses
    r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b',
    # Street addresses
    r'\b\d+\s+[A-Z][a-z]+\s+(Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Lane|Ln|Blvd|Boulevard|Way|Court|Ct)\b',
    # ZIP codes
    r'\b\d{5}(-\d{4})?\b',
    # SSN
...
class TranscriptManager:
    ...
    def save_log(self):
        if not SAVE_TRANSCRIPT:
            # Wipe transcript from memory — nothing is saved to disk
            self.full_log.clear()
            self.entries.clear()
            print("\nPrivacy mode: transcript wiped from memory. Nothing saved.")
            return
...
async def run_sales_pipeline():
    ...
    print("=" * 50)
    print("  PRIVACY MODE: ON  |  PCI COMPLIANCE: ON")
    print("  Conversations are NOT saved to disk")
    print("  PII is scrubbed before processing")
    print("=" * 50 + "\n")
    ...
    def on_transcript(data):
        ...
        try:
            ...
            if transcript.strip():
                ...
                if not pci_paused:
                    # Scrub PII before any processing or display
                    clean = scrub_pii(transcript)
                    print(f"Heard: {clean}")
                    transcript_manager.add(clean)
        except Exception as e:
